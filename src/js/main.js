...
require([
    ...
    //hljs
        ...
        var orgId = '';
        var arcGISservers = //JSON.parse("<config.predefinedArcGis>");
            [
                {
                    "name": "edn",
                    "portal": "https://gisweb7.ds.city.vancouver.bc.ca/arcgis/",
                    "server": "https://gisvm8.ds.city.vancouver.bc.ca/arcgis/"
                },
                {
                    "name": "uat",
                    "portal": "https://covgisstgport1.ds.city.vancouver.bc.ca/arcgis_portal/",
                    "server": "https://covgisstgport1.ds.city.vancouver.bc.ca/arcgis_server/"
                },
                {
                    "name": "prod",
                    "portal": "https://covgisweb1.ds.city.vancouver.bc.ca/arcgis_portal/",
                    "server": "https://covgisweb1.ds.city.vancouver.bc.ca/arcgis_server/"
                }]
        //arcGISservers.edn =
        //  {
        //     portal: 'https://gisweb7.ds.city.vancouver.bc.ca/arcgis/',
        ...
        var serverMappingPortal = function (portalUrl, testingUrl, type) {
            if (testingUrl == null) {
                return "good";
            } else {
                if (type == 'Map Service' || type == 'Feature Service' || type == 'Geometry Service' || type == 'Geoprocessing Service' || type == 'WMS') {
                    arcGISservers.forEach(function (gisServer) {
                        if (portalUrl.indexOf(gisServer.portal) >= 0)
                            return testingUrl.indexOf(gisServer.portal) < 0 ? "bad" : "good";
                    })
                    // if (portalUrl.indexOf(arcGISservers.edn.portal) >= 0) {
                    //     return testingUrl.indexOf(arcGISservers.edn.portal) < 0 ? "bad" : "good";
                    // }
                    // else if (portalUrl.indexOf(arcGISservers.uat.portal) >= 0) {
                    //     return testingUrl.indexOf(arcGISservers.uat.server) < 0 ? "bad" : "good";
                    // }
                    // else if (portalUrl.indexOf(arcGISservers.prod.portal) >= 0) {
                    //     return testingUrl.indexOf(arcGISservers.prod.server) < 0 ? "bad" : "good";
                    // }
                }
                else if (type == 'Code Attachment' || type == "Web Mapping Application") {
                    arcGISservers.forEach(function (gisServer) {
                        if (portalUrl.indexOf(gisServer.portal) >= 0)
                            return "good";
                    });
        ...
        var appInfo = new arcgisOAuthInfo({
            appId: "<config.appId>", // Set this in config from package.json
            popup: true,
            portalUrl: "<config.portalUrl>" // Set this in config.
        });
        ...
        arcGISservers.forEach(function (gisServer) {
            storePortal({ appId: '', portalUrl: gisServer.portal, usePkiIwa: false, useOauth: false, useUserPass: true });
        })
        // storePortal({ appId: '', portalUrl: arcGISservers.edn.portal, usePkiIwa: false, useOauth: false, useUserPass: true });
        // storePortal({ appId: '', portalUrl: arcGISservers.uat.portal, usePkiIwa: false, useOauth: false, useUserPass: true });
        // storePortal({ appId: '', portalUrl: arcGISservers.prod.portal, usePkiIwa: false, useOauth: false, useUserPass: true });
        ...
        var getFeatureServiceList = function (portal) {
            portal.search("type:Feature Service", 2000).then(function (items) {
                portal.allFeatures = items;
            })
        }

        var viewRef = function () {
            jquery(".content").click(function () {
                jquery("#dropArea").html('');
                // Display the selected Web Map's operational layers.
                var id = jquery(this).attr("data-id");
                //var webmapTitle = jquery(this).text();
                jquery(".content[data-type='Featue Service']").addClass("btn-info");
                jquery(".content").removeClass("active");
                jquery(".content").removeClass("btn-primary");
                jquery(this).addClass("btn-primary");
                jquery(this).removeClass("btn-info");
                ...
                var urlDependsTo = [];
                var tableDependsTo = [];
                portal.itemDependentsTo(id).then(function (dependends) {
                    jquery.each(dependends.list, function (item) {
                        if (this.dependencyType == 'id')
                            if (idDependsTo.indexOf(this.id) < 0)
                                idDependsTo.push(this.id)
                            else if (this.dependencyType == 'url') {
                                urlDependsTo.push(this.url);
                            }
                            else if (this.dependencyType == 'table') {
                                tableDependsTo.push(this.table);
                            }
                    });

                    var itemPromises = [];
                    var items = [];
                    jquery.each(idDependsTo, function (id) {
                        itemPromises.push(
                            portal.itemDescription(idDependsTo[id]).then(function (description) {
                                //var mapTitle = description.title;
                                items.push({
                                    id: description.id,
                                    title: description.title,
                                    owner: description.owner,
                                    ownerFolder: description.ownerFolder,
                    ...
                    Promise.all(itemPromises).then(function () {
                        var template = jquery("#viewRef").html();
                        var templateData = {};

                        var html = mustache.to_html(template, { maps: items });

                        // Add the HTML container with the item JSON.
                        jquery("#dropArea").html(html).promise().done(
                            function () {
                                jquery(".idRefMaps").click(function () {
                                    var id = $(this).attr('data');
                                    var indicatedItem = jquery('#' + id);
                                    $(indicatedItem.parent()).addClass("in").css('height', '');
                                    $(".indicated").removeClass("indicated");
                                    indicatedItem.addClass('indicated');
                                });
                            }
                        )
                    })
        ...
        var viewRef2 = function () {
            ...
            var supportedContent = jquery(".content[data-type='Feature Service']")
            ...
            jquery(".content").click(function () {
                ...
                jquery.each(jquery(".content[data-type='Web Map']"), function (map) {
                    var mapId = this.id;
                    //var owner = this.owner;
                    //var mapTitle = this.title;
                    itemPromises.push(
                        portal.itemDescription(mapId).then(function (description) {
                            var mapTitle = description.title;
                            items.push({
                                mapId: mapId,
                                mapTitle: mapTitle,
                ...
                itemPromises.push(
                    //portal.search("orgid:\"" + orgId + "\" AND (type:\"Web Map\" -type:\Web Mapping Application\") AND -OWNER:\"esri_*\"", 100)
                    portal.search("((-owner:esri_*) (-owner:" + portal.username + ")) (orgid:" + orgId + ") (type:\"Web Map\" -type:\"Web Mapping Application\")", 200)
                        .then(function (foundItems) {
                            jquery.each(foundItems.results, function (it) {
                                items.push({
                                    mapId: this.id,
                                    mapTitle: this.title,
                                    owner: this.owner,
                ...
                Promise.all(itemPromises).then(function () {
                    jquery.each(items, function (it) {
                        var mapId = this.mapId;
                        var mapTitle = this.mapTitle;
                        var owner = this.owner;
                        var ownerFolder = this.ownerFolder;
                        dataPromises.push(
                            portal.itemData(mapId).then(function (itemData) {
                                if (itemData) {
                                    if (itemData.operationalLayers && itemData.operationalLayers.length > 0) {
                                        jquery.each(itemData.operationalLayers, function (layer) {
                                            var layerId = this.itemId
                                            if (layerId == id) {
                                                var mapObj = { id: mapId, title: mapTitle, owner: owner, ownerFolder: ownerFolder };
                                                var temp = jquery.grep(maps, function (a) {
                                                    return a.id == mapId;
                                                });
                                                if (temp.length <= 0) {
                                                    maps.push(mapObj);
                    ...
                    Promise.all(dataPromises).then(function () {
                        ...
                        // jquery.each(maps, function(map){
                        //     console.log(this.id + '  ' + this.title);
                        // })
                        var template = jquery("#viewRef").html();
                        var templateData = {};

                        var html = mustache.to_html(template, { maps: maps });
        ...
        var updateWebmapServices = function () {
            ...
            var portal = app.portals.sourcePortal;
            var arcDefinedServers = arcGISservers;
            ...
            // Add a listener for clicking on content buttons.
            jquery(".content[data-type='Web Map']").click(function () {
                jquery("#dropArea").html('');
                // Display the selected Web Map's operational layers.
                var id = jquery(this).attr("data-id");
                var webmapTitle = jquery(this).text();
                ...
                portal.itemDescription(id)
                    .then(function (description) {
                        owner = description.owner;
                        privateUrl = !description.privateUrl ? "" : description.privateUrl;
                        itemUrl = !description.url ? "" : description.url;
                        thumbnail = !description.thumbnail ? "" : description.thumbnail;
                        itemType = description.type;
                        itemTitle = description.title;
                ...
                portal.itemData(id)
                    .then(function (data) {
                        //may not have data -nick
                        if (data == null) {
                            //jquery("#dropArea").html("No Data for this serice.")
                            //return;
                        }
                        else {
                            ////////////////////////////
                            webmapData = JSON.stringify(data);
                            if (itemType === 'Web Mapping Application') {
                                var mapId = data.map.itemId;
                                var mapPortalUrl = data.map.portalUrl;
                                var portalUrl = data.portalUrl;
                                var proxyUrl = data.httpProxy.url;

                                portal.search("type:Web Map AND owner:" + portal.username, 10).then(function (foundItems) {
                                    ;
                                });
                            }
                            else if (itemType == 'Feature Service') {
                                ...
                                var promises = []
                                jquery.each(operationalLayers, function (i) {
                                    var ind = this.url.indexOf('/rest/services/');
                                    var lastInd = this.url.lastIndexOf('/');
                                    var url = this.url;
                                    var id = this.id;
                                    ...
                                    // searchingType = 'Map Service';
                                    // else 
                                    if (layerType == 'ArcGISFeatureLayer')
                                        searchingType = 'Feature Service'

                                    if (ind > 0) {
                                        url = url.substring(0, lastInd);
                                    }
                                    var arcServer = url.substr(0, ind + 1);
                                    var isDefined = false;
                                    var isHosted = (this.url.indexOf("Hosted") > 0);
                                    if (isHosted) isDefined = true;
                                    else
                                        arcDefinedServers.forEach(function (gisServer) {
                                            if (arcServer == gisServer.Server && !isDefined) {
                                                isDefined = true;
                                            }
                                        })
                                    if (arcServer != portal.portalUrl && isDefined) {
                                        //isDefined = false;
                                        arcDefinedServers.forEach(function (gisServer) {

                                            if (portal.portalUrl == gisServer.portal && isDefined) {
                                                if (isHosted) {
                                                    url = url.replace(arcServer, gisServer.server);
                                                    isDefined = false;
                                                }
                                                else {
                                                    url = url.replace(arcServer, gisServer.server);
                                        ...
                                        // if (portal.portalUrl == arcDefinedServers.edn.portal)
                                        //     url = url.replace(arcServer, arcDefinedServers.edn.server);
                                        // else if (portal.portalUrl == arcDefinedServers.uat.portal)
                                        //     url = url.replace(arcServer, arcDefinedServers.uat.server)
                                        // else if (portal.portalUrl == arcDefinedServers.prod.portal)
                                        //     url = url.replace(arcServer, arcDefinedServers.prod.server)
                                    }
                                    //    url.replace(arcServer, arcDefinedServers.edn.server)
                                    operationalLayers[i].idCandidates = [];
                                    operationalLayers[i].display = 'none';
                                    //operationalLayers[i].owner = owner;
                                    if (searchingType == 'Feature Service')
                                        promises.push(
                                            portal.search(url + " AND type:" + searchingType, 10).then(function (foundItems) {
                                                operationalLayers[i].display = 'block';
                                                //operationalLayers[i].owner = this.owner;
                                                if (foundItems.total > 0) {
                                                    operationalLayers[i].newItemId = foundItems.results[0].id;
                                                    operationalLayers[i].multiple = false;

                                                    jquery.each(foundItems.results, function (j) {
                                                        operationalLayers[i].idCandidates.push({ id: foundItems.results[j].id, owner: foundItems.results[j].owner, title: foundItems.results[j].title, url: foundItems.results[j].url });
                                                    })
                                                    if (foundItems.total > 1) {
                                                        operationalLayers[i].multiple = true;
                                                    }
                                                }
                                                else {
                                                    operationalLayers[i].multiple = false;
                                                    operationalLayers[i].newItemId = "No such item exists!!"
                                                    //operationalLayers[i].ids ="";
                                                }
                                            })
                                        );
                                });
                                ...
                                Promise.all(promises).then(function () {
                                    ...
                                    jquery(".idCandidates").click(function () {
                                        var id = $(this).attr('data');
                                        ...
                                        var urlInput = $(this).parent().next().next();
                                        //urlInput.attr('data-original').val();
                                        urlInput.val(url);
                                    })
                                    ...
                                    jquery("[data-original]").change(function () {
                                        if ($(this).hasClass('bad')) {
                                            $(this).removeClass('bad');
                                        }
                                    });

                                    $('#btnValidateOpLayers').click(function () {
                                        var serviceUrls = jquery("[data-original]");
                                        var promises = [];
                                        var hasFail = false;
                                        var portal = app.portals.sourcePortal;
                                        var server = "XXXXX";
                                        jquery.each(arcGISservers, function (i) {
                                            var gisServer = arcGISservers[i];
                                            if (gisServer.portal == portal.portalUrl) {
                                                server = gisServer.server;
                                            }
                                        })

                                        jquery.each(serviceUrls, function (i) {

                                            var service = serviceUrls[i];

                                            if (service.id != '') {
                                                var url = jquery(service).val() + '?f=json';
                                                if (url.indexOf(server) == 0)
                                                    url += "&token=" + portal.token;
                                                //&token='+token;
                                                promises.push(
                                                    jquery.ajax(url)
                                                        .then(function (result) {

                                                            var r = typeof (result) == 'object' ? result : JSON.parse(result);
                                                            if (r.error) {
                                                                if (r.error.code == 499)
                                                                    jquery(service).addClass("tokenRequired");
                                                                else if (r.error.code == 500)
                                                                    jquery(service).addClass("bad")
                                                                else
                                                                    jquery(service).addClass("bad")
                                                                hasFail = true;
                                                            }
                                        ...
                                        Promise.all(promises)
                                            .then(function (results) {
                                                html = mustache.to_html(jquery("#updateSuccessTemplate").html());
                                                if (!hasFail) {
                                                    html = html.replace("Failed!", "Success!");
                                                }
                                                else {
                                                    html = html.replace("Success!", "Failed!");
                                                }
                                                jquery("#btnValidateOpLayers").before(html);
                                                jquery("#updateSuccessTemplate").hide(2000, function () {
                                                    html = html.replace("Failed!", "Success!");
                                                });
                                            }, function (errors) {
                                                html = mustache.to_html(jquery("#updateSuccessTemplate").html());
                                    ...
                                    jquery("#btnResetWebmapServices").click(function () {
                                        var webmapServices = jquery("[data-original]");
                                        jquery.each(webmapServices, function (i) {
                                            $service = jquery(webmapServices[i]);
                                            var originalUrl = $service.attr("data-original");
                                            $service.val(originalUrl);
                                        });
                                    });
                                });
                            }
        ...
        var updateContentUrls = function () {
            ...
            // Add a listener for clicking on content buttons.
            jquery(".content[data-type='Feature Service']").click(function () {
        ...
        //return candidate maps of current folder
        var getCandidateMaps = function (folderId, portal) {
            var deferred = new jquery.Deferred();
            var portalUrl = portal.portalUrl;
            var promises = [];
            var candidateMaps = [];
            jquery.each(jquery("#collapse_" + folderId + ">div.content[data-type='Web Map']"), function (map) {
                var mapId = this.id;
                //var owner = this.owner;
                //var mapTitle = this.title;

                promises.push(
                    portal.itemDescription(mapId)
                )
            });

            Promise.all(promises).then(function (descriptions) {
                jquery.each(descriptions, function (i) {
                    var description = descriptions[i];
                    candidateMaps.push({
                        mapId: description.id,
                        mapTitle: description.title,
                        owner: description.owner,
                        ownerFolder: description.ownerFolder,
                        cadidateMapPortal: portalUrl
                    })
                });

                deferred.resolve(candidateMaps);
            })

            return deferred.promise();
        };

        var updateWebmapAppServices = function () {
            var owner;
            var folder;
            var supportedContent = jquery(".content[data-type='Web Mapping Application'], .content[data-type='Mobile Application']");
            var portal = app.portals.sourcePortal;
            ...
            // Add a listener for clicking on content buttons.
            jquery(".content[data-type='Web Mapping Application']").click(function () {
                var deferred = new jquery.Deferred();
                // Display the selected item's URL.
                ...
                folder = jquery(this).parent().attr("data-folder");
                //jquery("#collapse_def748d9531b4aeaae92153a974f368a>div.content[data-type='Web Map']")
                ...
                promises.push(
                    portal.itemDescription(id).then(function (description) {
                        ...
                        //itemDesc.url = description.url;
                        //itemDesc.title = description.title;
                        //itemDesc.folder = folder;
                        owner = itemDesc.owner;
                        if (!itemDesc.ownerFolder) {
                            folder = ""; // Handle content in the user's root folder.
                        } else {
                            folder = itemDesc.ownerFolder;
                        }
                    })
                );
                ...
                Promise.all(promises).then(function () {
                    ...
                    if (itemData == null) {
                        ...
                        temp.geometryService = itemData.geometryService;
                        temp.httpProxy = itemData.httpProxy;
                        temp.authorizedCrossOriginDomains = itemData.authorizedCrossOriginDomains;
                        temp.portalUrl = itemData.portalUrl;
                        temp.widgetPool = { widgets: [] };
                        temp.widgetOnScreen = { widgets: [] };
                        jquery.each(itemData.widgetPool.widgets, function (i) {
                            var widgetItem = itemData.widgetPool.widgets[i];
                            if (widgetItem.uri != null)
                                if (widgetItem.uri.indexOf("http") == 0 || widgetItem.uri.indexOf("//") == 0) {
                                    temp.widgetPool.widgets.push({
                                        id: widgetItem.id,
                                        uri: widgetItem.uri,
                        ...
                        jquery.each(itemData.widgetOnScreen.widgets, function (i) {
                            var widgetItem = itemData.widgetOnScreen.widgets[i];
                            if (widgetItem.uri != null)
                                if (widgetItem.uri.indexOf("http") == 0 || widgetItem.uri.indexOf("//") == 0) {
                                    temp.widgetOnScreen.widgets.push({
                                        id: widgetItem.id,
                                        uri: widgetItem.uri,
                                        name: widgetItem.name
                                    });
                        ...
                        temp.map = itemData.map == null ? null :
                            {
                                id: itemData.map.id,
                                portalUrl: itemData.portalUrl,
                                itemId: itemData.map.itemId//,
                                //mapPortalUrl: itemData.map.portalUrl
                            }
                        getCandidateMaps(folder, portal).then(function (candidateMaps) {
                            temp.map.idCandidates = candidateMaps;
                            deferred.resolve();
                        })
                    }

                    deferred.then(function () {
                        var html = mustache.to_html(jquery("#webMapAppTemplate").html(), temp);
                        ...
                        jquery(".candidateMap").click(function () {
                            var $this = jquery(this);
                            var newMapId = $this.attr('data');
                            var dataUrl = $this.attr('data-url');

                            var indicatedItem = jquery('#' + newMapId);
                            $(indicatedItem.parent()).addClass("in").css('height', '');

                            $(".indicated").removeClass("indicated");
                            indicatedItem.addClass('indicated');

                            jquery('#mapItemId').val(newMapId);
                            jquery("#mapUrl").val(dataUrl);
                        })

                        // Event listener for update button.
                        jquery("#btnUpdateContentUrl").click(function () {
                            var contentId = jquery(".content.active.btn-primary").attr("data-id");
                            var url = jquery("#appUrl").val();

                            portal.updateUrl(owner, folder, contentId, url).then(function (response) {
                                var html;
                                if (response.success) {

                                    jquery("[appUrl]").attr("data-original", url);
                                    ...
                                    if (itemData == null) {
                                        ...
                                        var mapId = jquery("#mapItemId").val();
                                        var mapUrl = jquery("#mapUrl").val();

                                        itemData.map.itemId = mapId;
                                        itemData.map.portalUrl = mapUrl;
                                        itemData.portalUrl = $("#appPortalUrl").val();
                                        itemData.geometryService = $("#appGeometryService").val();
                                        itemData.authorizedCrossOriginDomains = $('#appAuthorizedCRODs').val();
                                        itemData.httpProxy.url = $("#appHttpProxyUrl").val();


                                        jquery.each(itemData.widgetPool.widgets, function (i) {
                                            var widget = itemData.widgetPool.widgets[i];
                                            var updatedWidget = $("#" + widget.id);
                                            if (updatedWidget.length > 0) {
                                                widget.uri = $(updatedWidget.children()[1]).val();
                                            }
                                        });

                                        jquery.each(itemData.widgetOnScreen.widgets, function (i) {
                                            var widget = itemData.widgetOnScreen.widgets[i];
                                            var updatedWidget = $("#" + widget.id);
                                            if (updatedWidget.length > 0) {
                                                widget.uri = $(updatedWidget.children()[1]).val();
                                            }
                                        ...
                                        portal.updateWebmapData(owner, folder, itemId, itemData).then(function (res) {
                                            if (res.success) {
                                                //jquery("[mapUrl]").attr("data-original", mapUrl);

                                                var originalData = jquery('[data-original]');
                                                jquery.each(originalData, function (i) {
                                                    var $original = jquery(originalData[i]);

                                                    $original.attr("data-original", $original.val());
                                                })
                        ...
                        jquery("#btnResetContentUrl").click(function () {
                            ...
                            jquery.each(originalData, function (i) {
                                ...
                                var originalUrl = $original.attr("data-original");
            ...
            jquery(document).on("click", "li [data-action]", function (e) {
                ...
                switch (selectedAction) {
                    ...
                    case "viewRef":
                        ...
                        viewRef();
                        break;
                    case "updateWebmapAppServices":
                        cleanUp();
                        updateWebmapAppServices();
                        break;
                }
            });
